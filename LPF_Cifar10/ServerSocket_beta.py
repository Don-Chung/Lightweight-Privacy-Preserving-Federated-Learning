...
from Initial import gen_secret, para, port_user_beta, port_alpha_beta, init_cnn_beta, para_dynamic, Logger, \
    gen_secret_join, dynamic_number, dynamic_iod, init_cnn_beta_r
...
while True:
    ...
    features_40_weight, features_40_bias, features_41_weight, features_41_bias, classifier_weight, classifier_bias = init_cnn_beta()
    ...
    features_r40_weight, features_r40_bias, features_r41_weight, features_r41_bias, classifier_rweight, classifier_rbias = init_cnn_beta_r()
    ...
    for i in range(user_number):
        ...
        features_r0_weight += (- secret_seed(seed_1[i], decode[0].shape[0], decode[0].shape[1]-1)) * (
                    size[i] / total_size)
        features_r0_bias += (- secret_seed(seed_1[i], decode[1].shape[0], 0)) * (size[i] / total_size)
        features_r1_weight += (- secret_seed(seed_1[i], decode[2].shape[0], 0)) * (size[i] / total_size)
        features_r1_bias += (- secret_seed(seed_1[i], decode[3].shape[0], 0)) * (size[i] / total_size)
        features_r3_weight += (- secret_seed(seed_1[i], decode[4].shape[0], decode[4].shape[1]-1)) * (
                    size[i] / total_size)
        features_r3_bias += (- secret_seed(seed_1[i], decode[5].shape[0], 0)) * (size[i] / total_size)
        features_r4_weight += (- secret_seed(seed_1[i], decode[6].shape[0], 0)) * (size[i] / total_size)
        features_r4_bias += (- secret_seed(seed_1[i], decode[7].shape[0], 0)) * (size[i] / total_size)

        features_r7_weight += (- secret_seed(seed_1[i], decode[8].shape[0], decode[8].shape[1]-1)) * (
                    size[i] / total_size)
        features_r7_bias += (- secret_seed(seed_1[i], decode[9].shape[0], 0)) * (size[i] / total_size)
        features_r8_weight += (- secret_seed(seed_1[i], decode[10].shape[0], 0)) * (size[i] / total_size)
        features_r8_bias += (- secret_seed(seed_1[i], decode[11].shape[0], 0)) * (size[i] / total_size)
        features_r10_weight += (- secret_seed(seed_1[i], decode[12].shape[0], decode[12].shape[1]-1)) * (
                    size[i] / total_size)
        features_r10_bias += (- secret_seed(seed_1[i], decode[13].shape[0], 0)) * (size[i] / total_size)
        features_r11_weight += (- secret_seed(seed_1[i], decode[14].shape[0], 0)) * (size[i] / total_size)
        features_r11_bias += (- secret_seed(seed_1[i], decode[15].shape[0], 0)) * (size[i] / total_size)

        features_r14_weight += (- secret_seed(seed_1[i], decode[16].shape[0], decode[16].shape[1]-1)) * (
                    size[i] / total_size)
        features_r14_bias += (- secret_seed(seed_1[i], decode[17].shape[0], 0)) * (size[i] / total_size)
        features_r15_weight += (- secret_seed(seed_1[i], decode[18].shape[0], 0)) * (size[i] / total_size)
        features_r15_bias += (- secret_seed(seed_1[i], decode[19].shape[0], 0)) * (size[i] / total_size)
        features_r17_weight += (- secret_seed(seed_1[i], decode[20].shape[0], decode[20].shape[1]-1)) * (
                    size[i] / total_size)
        features_r17_bias += (- secret_seed(seed_1[i], decode[21].shape[0], 0)) * (size[i] / total_size)
        features_r18_weight += (- secret_seed(seed_1[i], decode[22].shape[0], 0)) * (size[i] / total_size)
        features_r18_bias += (- secret_seed(seed_1[i], decode[23].shape[0], 0)) * (size[i] / total_size)

        features_r20_weight += (- secret_seed(seed_1[i], decode[24].shape[0], decode[24].shape[1]-1)) * (
                    size[i] / total_size)
        features_r20_bias += (- secret_seed(seed_1[i], decode[25].shape[0], 0)) * (size[i] / total_size)
        features_r21_weight += (- secret_seed(seed_1[i], decode[26].shape[0], 0)) * (size[i] / total_size)
        features_r21_bias += (- secret_seed(seed_1[i], decode[27].shape[0], 0)) * (size[i] / total_size)
        features_r24_weight += (- secret_seed(seed_1[i], decode[28].shape[0], decode[28].shape[1]-1)) * (
                    size[i] / total_size)
        features_r24_bias += (- secret_seed(seed_1[i], decode[29].shape[0], 0)) * (size[i] / total_size)
        features_r25_weight += (- secret_seed(seed_1[i], decode[30].shape[0], 0)) * (size[i] / total_size)
        features_r25_bias += (- secret_seed(seed_1[i], decode[31].shape[0], 0)) * (size[i] / total_size)

        features_r27_weight += (- secret_seed(seed_1[i], decode[32].shape[0], decode[32].shape[1]-1)) * (
                    size[i] / total_size)
        features_r27_bias += (- secret_seed(seed_1[i], decode[33].shape[0], 0)) * (size[i] / total_size)
        features_r28_weight += (- secret_seed(seed_1[i], decode[34].shape[0], 0)) * (size[i] / total_size)
        features_r28_bias += (- secret_seed(seed_1[i], decode[35].shape[0], 0)) * (size[i] / total_size)
        features_r30_weight += (- secret_seed(seed_1[i], decode[36].shape[0], decode[36].shape[1]-1)) * (
                    size[i] / total_size)
        features_r30_bias += (- secret_seed(seed_1[i], decode[37].shape[0], 0)) * (size[i] / total_size)
        features_r31_weight += (- secret_seed(seed_1[i], decode[38].shape[0], 0)) * (size[i] / total_size)
        features_r31_bias += (- secret_seed(seed_1[i], decode[39].shape[0], 0)) * (size[i] / total_size)

        features_r34_weight += (- secret_seed(seed_1[i], decode[40].shape[0], decode[40].shape[1]-1)) * (
                    size[i] / total_size)
        features_r34_bias += (- secret_seed(seed_1[i], decode[41].shape[0], 0)) * (size[i] / total_size)
        features_r35_weight += (- secret_seed(seed_1[i], decode[42].shape[0], 0)) * (size[i] / total_size)
        features_r35_bias += (- secret_seed(seed_1[i], decode[43].shape[0], 0)) * (size[i] / total_size)
        features_r37_weight += (- secret_seed(seed_1[i], decode[44].shape[0], decode[44].shape[1]-1)) * (
                    size[i] / total_size)
        features_r37_bias += (- secret_seed(seed_1[i], decode[45].shape[0], 0)) * (size[i] / total_size)
        features_r38_weight += (- secret_seed(seed_1[i], decode[46].shape[0], 0)) * (size[i] / total_size)
        features_r38_bias += (- secret_seed(seed_1[i], decode[47].shape[0], 0)) * (size[i] / total_size)

        features_r40_weight += (- secret_seed(seed_1[i], decode[48].shape[0], decode[48].shape[1]-1)) * (
                    size[i] / total_size)
        features_r40_bias += (- secret_seed(seed_1[i], decode[49].shape[0], 0)) * (size[i] / total_size)
        features_r41_weight += (- secret_seed(seed_1[i], decode[50].shape[0], 0)) * (size[i] / total_size)
        features_r41_bias += (- secret_seed(seed_1[i], decode[51].shape[0], 0)) * (size[i] / total_size)
        classifier_rweight += (- secret_seed(seed_1[i], decode[52].shape[0], decode[52].shape[1])) * (
                    size[i] / total_size)
        classifier_rbias += (- secret_seed(seed_1[i], decode[53].shape[0], 0)) * (size[i] / total_size)
    ...
    features_0_weight = np.hstack((features_r0_weight, features_0_weight))
    features_0_bias = np.hstack((features_r0_bias, features_0_bias))
    features_1_weight = np.hstack((features_r1_weight, features_1_weight))
    features_1_bias = np.hstack((features_r1_bias, features_1_bias))
    features_3_weight = np.hstack((features_r3_weight, features_3_weight))
    features_3_bias = np.hstack((features_r3_bias, features_3_bias))
    features_4_weight = np.hstack((features_r4_weight, features_4_weight))
    features_4_bias = np.hstack((features_r4_bias, features_4_bias))

    features_7_weight = np.hstack((features_r7_weight, features_7_weight))
    features_7_bias = np.hstack((features_r7_bias, features_7_bias))
    features_8_weight = np.hstack((features_r8_weight, features_8_weight))
    features_8_bias = np.hstack((features_r8_bias, features_8_bias))
    features_10_weight = np.hstack((features_r10_weight, features_10_weight))
    features_10_bias = np.hstack((features_r10_bias, features_10_bias))
    features_11_weight = np.hstack((features_r11_weight, features_11_weight))
    features_11_bias = np.hstack((features_r11_bias, features_11_bias))

    features_14_weight = np.hstack((features_r14_weight, features_14_weight))
    features_14_bias = np.hstack((features_r14_bias, features_14_bias))
    features_15_weight = np.hstack((features_r15_weight, features_15_weight))
    features_15_bias = np.hstack((features_r15_bias, features_15_bias))
    features_17_weight = np.hstack((features_r17_weight, features_17_weight))
    features_17_bias = np.hstack((features_r17_bias, features_17_bias))
    features_18_weight = np.hstack((features_r18_weight, features_18_weight))
    features_18_bias = np.hstack((features_r18_bias, features_18_bias))

    features_20_weight = np.hstack((features_r20_weight, features_20_weight))
    features_20_bias = np.hstack((features_r20_bias, features_20_bias))
    features_21_weight = np.hstack((features_r21_weight, features_21_weight))
    features_21_bias = np.hstack((features_r21_bias, features_21_bias))
    features_24_weight = np.hstack((features_r24_weight, features_24_weight))
    features_24_bias = np.hstack((features_r24_bias, features_24_bias))
    features_25_weight = np.hstack((features_r25_weight, features_25_weight))
    features_25_bias = np.hstack((features_r25_bias, features_25_bias))

    features_27_weight = np.hstack((features_r27_weight, features_27_weight))
    features_27_bias = np.hstack((features_r27_bias, features_27_bias))
    features_28_weight = np.hstack((features_r28_weight, features_28_weight))
    features_28_bias = np.hstack((features_r28_bias, features_28_bias))
    features_30_weight = np.hstack((features_r30_weight, features_30_weight))
    features_30_bias = np.hstack((features_r30_bias, features_30_bias))
    features_31_weight = np.hstack((features_r31_weight, features_31_weight))
    features_31_bias = np.hstack((features_r31_bias, features_31_bias))

    features_34_weight = np.hstack((features_r34_weight, features_34_weight))
    features_34_bias = np.hstack((features_r34_bias, features_34_bias))
    features_35_weight = np.hstack((features_r35_weight, features_35_weight))
    features_35_bias = np.hstack((features_r35_bias, features_35_bias))
    features_37_weight = np.hstack((features_r37_weight, features_37_weight))
    features_37_bias = np.hstack((features_r37_bias, features_37_bias))
    features_38_weight = np.hstack((features_r38_weight, features_38_weight))
    features_38_bias = np.hstack((features_r38_bias, features_38_bias))

    features_40_weight = np.hstack((features_r40_weight, features_40_weight))
    features_40_bias = np.hstack((features_r40_bias, features_40_bias))
    features_41_weight = np.hstack((features_r41_weight, features_41_weight))
    features_41_bias = np.hstack((features_r41_bias, features_41_bias))
    classifier_weight = np.hstack((classifier_rweight, classifier_weight))
    classifier_bias = np.hstack((classifier_rbias, classifier_bias))
